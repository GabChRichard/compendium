# Examen Pratique : Le montage

Travail individuelle *(15% de la note finale)*

## Consignes

À la maison, faire le montage d'un générique d'ouverture avec les médias fournis. 

### Étapes

* Vérifier si vous avez déjà les médias sur votre disque dure. 

    * Dans le dossier **da_vinci_votre_nom**

    * Dans le dossier **devoir**

    * Dans le dossier **tp_générique_ouverture**

    > Si vous n'avez pas les médias nécessaire, vous pouvez les télécharger [ici](https://cmontmorency365-my.sharepoint.com/:f:/g/personal/dominic_roberts_cmontmorency_qc_ca/IgAeDVmWJa2iTLCbnTnt8K9AAeuWtihukNyTEHr_qZq0U-k?e=B7pmqU)

* Faire le montage du générique sur la musique "theme_main.wav"

* Ajouter un générique de fin qui doit inclure : 

    1. Nom des intervenants

    2. Sources

    3. Encadrement

    4. Remerciements

    5. Logo TIM

### Le dossier de remise doit comprendre les éléments suivants :

- Votre rendu finale. 
- Votre fichier drp.
- Des captures d'écran de l'organisation des dossiers dans votre dossier de travail. 

## Critères d'évaluation

Les aspects évalués sont les suivants :

| Catégorie | Sous-critère | Points |
|---|---|---|
| **Qualité technique du montage** | | **/10** |
| | Organisation du dossier de travail | /10 |
| **Qualité technique du montage** | | **/40** |
| | Synchronisation son/image | /20 |
| | Fluidité du montage | /15 |
| | Niveau sonore | /5 |
| **Qualité esthétique du montage** | | **/50** |
| | Rythme | /10 |
| | Structure | /10 |
| | Transitions | /10 |
| | Effets | /10 |
| | Générique de fin | /10 |
| **Total** | | **/100** |

> Un manquement au respect du format et à la présence de tous les éléments sera pénalisé.

> N'oubliez pas que la qualité de la langue est évaluée jusqu'à concurrence de 10% de la note finale du travail (1% par faute).
